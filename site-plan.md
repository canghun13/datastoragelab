...
## 16. PostgreSQL Maintenance Storage Planning expansion — 2026-08-31

The completed 94-page inventory is extended to 101 public pages: 56 tools, 15 guides, eight references, four comparisons, 13 hubs, and five foundational pages. The extension adds H13 **PostgreSQL Maintenance Storage Planning** (`/tools/postgresql-storage/`); T53 **PostgreSQL VACUUM FULL Disk Space Checker**; T54 **PostgreSQL Index Build & Reindex Space Planner**; T55 **PostgreSQL WAL Retention & Replication Slot Planner**; T56 **PostgreSQL Base Backup Storage & Transfer Planner**; G15 **How to Plan PostgreSQL Maintenance Storage**; and R08 **PostgreSQL Maintenance Storage Formulas**.

This cluster owns temporary and retained storage boundaries created by PostgreSQL maintenance operations. T53 compares an explicitly estimated rewritten table, TOAST, and index working set with filesystem free space. T54 budgets new index output, simultaneous builds, temporary work, and concurrent growth. T55 converts measured WAL generation and consumer lag into retained bytes and time to disk or slot-cap boundaries. T56 solves the feedback between base-backup duration and WAL produced while a shared stream remains active. These decisions differ from previously rejected generic database storage sizing because they begin with measured operation evidence and end with a go/no-go preflight or response deadline, rather than forecasting the steady database footprint.

H13 follows Developer Build Storage in the Header Tools menu and Tools directory. G15 and R08 appear in the existing Learn indexes. Home exposes a PostgreSQL maintenance scenario without changing the protected directory-badge area. Contextual links connect backup verification, backup-window, and VM/container endurance workflows to the relevant database operation. All table sizes, index outputs, temporary allowances, free-space reserves, WAL rates, slot caps, payloads, and throughput values remain user-entered. The implementation embeds no server access, provider policy, version-specific size estimate, managed-service quota, or product database.
